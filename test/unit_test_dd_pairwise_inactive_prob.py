...
logging.basicConfig(level=logging.INFO,\
        format='%(message)s',\
        filename=output_folder+'/result_dd_pairwise_inactive_prob.log', filemode='w')
...
def main():    
    ...
    for Nr in [2,4,10]:
        ...
        exp_X = np.exp(-kappa*X)
        pairwise_zero = exp_X.T.dot(exp_X)/Nc
        np.fill_diagonal(pairwise_zero,np.mean(exp_X,axis=0))
        logger.info(str(pairwise_zero))
        logger.info('')
        ...
        start_time = time.time()
        p0_ml,p0_dd  = sd.dd_pairwise_inactive_prob(data,size_factor=size_factor,relative_depth=Nr/kappa,verbose=False)
        logger.info('## p0_ml ')
