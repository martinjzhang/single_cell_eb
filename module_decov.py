...
## data generation
def data_gen_poi(p,x,N_c=1000,N_r=10,return_data=0):
    n_supp=p.shape[0]
    x=x*N_r
    lam=np.random.choice(x,N_c, p=p,replace=True)
...
def dd_1d(Y_pdf,Y_supp,x,N_c,N_r):
    ## parameter setting    
    n_supp=x.shape[0]
    n_obs=Y_pdf.shape[0]
    delta=1e-2
    ...
    #epsilon=np.sqrt(3*np.log(2/delta)/N_c/Y_pdf.clip(min=min_clamp))
    epsilon=np.sqrt(np.log(1/delta)/N_c/Y_pdf.clip(min=min_clamp)).clip(max=1)
    ci=epsilon*Y_pdf.clip(min=min_clamp)
    Y_pdf_ub=(Y_pdf+ci).clip(max=1)
    Y_pdf_lb=(Y_pdf-ci).clip(min=0)
...
def binomial_ci_cp(Y,N_c,delta):
    """
    ...
    if Y==0:
        ...
        lb=(delta/2.0)**(1/(N_c+0.0))
        ub=1
    else:
        lb=sp.stats.beta.ppf(delta/2, Y, N_c-Y+1)
        ub=sp.stats.beta.ppf(1 - delta/2, Y+1, N_c-Y)
    return lb, ub
